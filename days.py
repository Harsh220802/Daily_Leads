...
def get_refresh_interval():
    if 'refresh_interval' not in st.session_state:
        st.session_state.refresh_interval = 300  # Default 5 minutes (300 seconds)
...
def should_refresh():
    ...
    now = datetime.now()
    time_diff = (now - st.session_state.last_refresh_time).total_seconds()
...
selected_refresh = st.sidebar.selectbox(
    ...
    index=2  # Default to "Every 5 minutes"
)
st.session_state.refresh_interval = refresh_options[selected_refresh]

# Force refresh button
load_button = st.sidebar.button("Refresh Data Now")

# Display last refresh time
if 'last_refresh_time' in st.session_state:
    st.sidebar.caption(f"Last refreshed: {st.session_state.last_refresh_time.strftime('%Y-%m-%d %H:%M:%S')}")

st.title("📊 Leads Analysis Dashboard")

# Check if we should auto-refresh or force refresh
should_load = load_button or should_refresh() or 'data_loaded' not in st.session_state
